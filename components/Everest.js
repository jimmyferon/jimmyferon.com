...
const PITCH_MIN = -1.05;  // regard vers le haut : les sommets depuis un creux
...
export default function Everest() {
  ...
  useEffect(() => {
    ...
    withThree(() => {
      ...
      let camGround = 0;                               // sol sous la caméra, lissé
      ...
      const frame = (now) => {
        ...
        /* caméra : deux axes. Vers le bas, elle monte et plonge. Vers le haut,
           elle s'abaisse un peu et surtout lève le regard : c'est le point visé
           qui grimpe, pour voir les sommets même depuis un creux. Le sol sous
           la caméra est lissé, sinon chaque bosse la faisait sursauter. */
        const up = Math.max(0, -pitch), down = Math.max(0, pitch);
        const cd = CAM_D * Math.cos(down);
        const cx = pos.x + Math.cos(yaw) * cd;
        const cz = pos.z + Math.sin(yaw) * cd;
        const cy = pos.y + CAM_H * (1 - up * 0.72) + Math.sin(down) * CAM_D;
        const gTarget = groundY(cx, cz) + 2.4;
        camGround = camGround === 0 ? gTarget : camGround + (gTarget - camGround) * ease(3);
        camWant.set(cx, Math.max(cy, camGround), cz);
        ...
        if (go) {
          ...
          lookWant.set(pos.x, pos.y + FIG_H * 0.6 + up * 16, pos.z);
          lookCur.lerp(lookWant, ease(camK * 1.1));
        }
        camera.lookAt(lookCur);
