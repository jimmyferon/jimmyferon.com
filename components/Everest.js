...
const PITCH_MIN = -0.78;  // regard vers le haut (caméra basse)
...
export default function Everest() {
  ...
  const HELP = [
    ["ZQSD", " " + t("ev.hMoveSep") + " " + t("ev.hArrows") + " ", t("ev.hMove")],
    [t("ev.hCam"), " ", t("ev.hMouse")],
    [t("ev.hFlag"), " ", t("ev.hProject")],
  ];
  ...
  return (
    <div className="ev" ref={wrapRef}>
      ...
      <div className={"ev-help" + (ready && !touched ? " on" : "")} aria-hidden="true">
        {HELP.map((l) => (
          <span key={l[0]}><b>{l[0]}</b>{l[1]}<span className="ev-eq">=</span>{l[2]}</span>
        ))}
      </div>
